(async function () {
  ...
  // ── Guard: prevent the script from injecting twice on the same page ────────
  if (document.getElementById('gdn-root')) return;
  ...
  // Flag that stops our own storage listener from re-rendering after we save.
  // Without this, save() → onChanged fires → render() → save() → infinite loop.
  let _saving = false;

  // Persist the current state object to extension-global storage.
  function save() {
    _saving = true;
  ...
  // When state changes in ANOTHER tab, pull it in and re-render here too.
  // This keeps all open tabs in sync without any polling.
  browser.storage.onChanged.addListener((changes, area) => {
    if (_saving || area !== 'local' || !changes[STORAGE_KEY]) return;
    state = Object.assign({}, state, changes[STORAGE_KEY].newValue);
    render();
  });
  ...
  // Load persisted state from extension storage before first render.
  // browser.storage.local is shared across all tabs and all websites,
  // unlike localStorage which resets on every new origin.
  try {
    const result = await browser.storage.local.get(STORAGE_KEY);
    if (result[STORAGE_KEY]) state = Object.assign(state, result[STORAGE_KEY]);
  } catch (_) {}
